module github.com/rootwarp/ino-vibe-go-sdk

go 1.14

require (
	bitbucket.org/ino-on/ino-vibe-api v0.0.0-20200518140315-137a417e1f36
	cloud.google.com/go v0.57.0 // indirect
	github.com/bojand/ghz v0.52.0
	github.com/golang/protobuf v1.4.2
	github.com/grpc-ecosystem/grpc-gateway v1.14.6 // indirect
	github.com/stretchr/objx v0.2.0 // indirect
	github.com/stretchr/testify v1.6.0
	github.com/vektra/mockery v1.0.0 // indirect
	golang.org/x/net v0.0.0-20200528225125-3c3fba18258b // indirect
	golang.org/x/oauth2 v0.0.0-20200107190931-bf48bf16ab8d
	golang.org/x/sys v0.0.0-20200523222454-059865788121 // indirect
	golang.org/x/tools v0.0.0-20200507020122-88e38c1d8d1c // indirect
	google.golang.org/genproto v0.0.0-20200528191852-705c0b31589b // indirect
	google.golang.org/grpc v1.29.1
	gopkg.in/yaml.v2 v2.3.0 // indirect
	gopkg.in/yaml.v3 v3.0.0-20200506231410-2ff61e1afc86 // indirect
)
